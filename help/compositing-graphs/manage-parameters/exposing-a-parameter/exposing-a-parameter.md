...
![Exposing parameters simplified](exposing-a-parameter.resources/parameters-5.png "Exposing parameters simplified")
...
![Single expose method walkthrough](exposing-a-parameter.resources/single-expose2.gif "Single expose method walkthrough"){width="512px"}
...
1. Click the ![](exposing-a-parameter.resources/image2020-9-17-15-35-59.png) dropdown options button
1. Choose ![](exposing-a-parameter.resources/image2020-9-17-15-37-7.png) <b>Expose as new graph input</b> from the drop down list, the first option.
...
1. The parameter's name turns *blue*, and the ![](exposing-a-parameter.resources/image2020-9-17-15-35-46.png)  
...
![Batch expose method walkthrough](exposing-a-parameter.resources/batch-expose-2.gif "Batch expose method walkthrough"){width="512px"}
...
1. Instead of finding a single parameter, find the ![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png) <b>Multi-expose</b> button at the top right of the <b>Specific parameters</b> tab
...
1. Click <b>OK</b> to confirm and expose all selected parameters. The parameter names now show *blue* to confirm the parameters are exposed, as well as an ![](exposing-a-parameter.resources/image2020-9-17-15-35-46.png) <b>Edit function</b> button.
...
1. Click on the Dropdown options button ![](exposing-a-parameter.resources/image2020-9-17-15-35-59.png) next to the already exposed parameter
1. Choose ![](exposing-a-parameter.resources/image2020-9-17-15-37-7.png)<b> Edit exposed graph input</b>. This takes you straight to the relevant entry in the graph properties
...
![Input parameters](exposing-a-parameter.resources/input-parameters-2.png "Input parameters"){width="512px"}
...
![Drop down list editor for Integer parameter](exposing-a-parameter.resources/dropdown.gif "Drop down list editor for Integer parameter"){width="512px"}
...
![Re-order input parameters](exposing-a-parameter.resources/reorder-2.gif "Re-order input parameters"){width="512px"}
...
![Previewing input parameters](exposing-a-parameter.resources/parameter-preview-2.gif "Previewing input parameters"){width="512px"}
...
![Copying and pasting inputs parameters](exposing-a-parameter.resources/copy-paste-params-2.gif "Copying and pasting inputs parameters"){width="512px"}

### COPY-PASTING PARAMETERS

Parameters can be copy-pasted between graphs.

A single parameter can be copied with the Copy button ![](exposing-a-parameter.resources/image2019-9-19-11-3-49.png). Multiple Parameters can be copied through the Parameters Menu ![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png). Choose Copy Inputs to copy all inputs.

Choose Paste Inputs ![](exposing-a-parameter.resources/image2020-9-17-16-43-15.png) in the Parameters Menu ![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png) to paste one or more parameters.
...
![Errors on node parameters](exposing-a-parameter.resources/parameter-error.gif "Errors on node parameters"){width="512px"}

### TRACKING BROKEN PARAMETERS ON NODES

You can track what parameter is used by which node through the Node Finder Tool ![](exposing-a-parameter.resources/image2019-9-19-14-15-53.png), which is located in the top bar of the Graph View. Click it and it allows you to find nodes using specific parameters.

If a node has an actual problem, it will display a Warning badge ![](exposing-a-parameter.resources/image2019-9-19-14-23-54.png) on its top left corner. Hovering over the badge will display a tooltip with more information.

To reset and remove a problem, for the parameter you want to fix or reset, click on the Dropdown button ![](exposing-a-parameter.resources/image2020-9-17-15-35-59.png) next to the Edit Function button and select ![](exposing-a-parameter.resources/image2020-9-17-16-56-18.png) <b>Reset. </b>This returns a parameter to its previous, non-exposed state, the blue name will turn gray again to reflect this.

![Cleaning unused input parameters](exposing-a-parameter.resources/clean-inputs-2.gif "Cleaning unused input parameters"){width="512px"}

### CLEANING UP UNUSED INPUT PARAMETERS

If you have lost track of your Input Parameters and no longer know which ones are used, they can be cleaned up using a small tool. Click the Input Parameter menu button ![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png) and select <b>Clean Inputs.</b>
...
![Removing parameters](exposing-a-parameter.resources/delete-param-2.gif "Removing parameters"){width="512px"}

### REMOVING PARAMETERS

To actually remove a parameter that is in use, requires two distinct steps.

1. On the node with the exposed parameter, click the dropdown arrow to the right of the Function Expose button that is colored in blue: ![](exposing-a-parameter.resources/image2019-9-19-14-55-55.png). Then choose "Reset to Default Value". This removes the use of the parameter on this one node. repeat for any other node making use of the same parameter. "Reset to Default value" also resets the range of the parameter widget to its *soft range*.
