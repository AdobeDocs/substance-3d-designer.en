---
helpx_url: "https://helpx.adobe.com/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/texture-generators/noises/grunge-map-002.html"
breadcrumb-title: ""
description: Use the Grunge Map 002 node to generate procedural grunge patterns for creating weathered and aged material surfaces.
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Texture Generators > Noises > Grunge Map 002
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Grunge Map 002
user-guide-description: ""
user-guide-title: ""
---


# Grunge Map 002

<table>
<tr style="border: 0;">
<td style="border: 0;" valign="top">

![](grungemap-002.png){width="128px"}

## Grunge Map 002

**In:** *Texture Generators**/Noises*

**Simple**

</td>
<td style="border: 0;" valign="top">

## Description

Generates a complex, combined Noisemap. This node can be very useful as a detailed procedural, but keep in mind these are very performance-intensive and thus slower to generate.

## Parameters

* **Balance**: *0.0 - 1.0*  
  Shifts the balance of the result between black or white, like a brightness adjustment.
* **Contrast**: *0.0 - 1.0*  
  Adjusts the contrast of the result.
* **Invert**: *False/True*  
  Inverts the result.
* **Brush Pattern**: *0.0 - 1.0*Adds a mask around the edges, for when used as a brush alpha.
* **Non Square Expansion**: *False/True*  
  Enables compensation of squash and stretch with non-square ratios.

## Example Images

![](grungemap-002-ex.gif)

</td>
</tr>
</table>
