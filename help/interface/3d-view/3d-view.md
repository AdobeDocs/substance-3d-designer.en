...
<code>LMB</code> Select a mesh in the scene.
...
A contextual menu is available for selected meshes and may be displayed by clicking <code>RMB</code>.

Meshes can also be selected in Camera or Light modes, by pressing <code>Shift+LMB</code>.
...
<code>LMB</code> Orbit the camera around its target.<br><code>RMB</code> Move the camera closer or farther from its target.
...
<code>LMB</code> Orbit the camera around the scene's origin.<br><code>RMB</code> Move the light closer or farther from the scene's origin.
