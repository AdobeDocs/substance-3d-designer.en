---
helpx_url: "https://helpx.adobe.com/substance-3d-designer/interface/the-main-toolbar.html"
breadcrumb-title: ""
description: Learn about the main toolbar in Substance 3D Designer to access common tools and commands for your workflow.
helpx_creative_field: ""
helpx_description: Designer > Interface > Main toolbar
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Main toolbar
user-guide-description: ""
user-guide-title: ""
---

# The main toolbar

<table>
<tr style="border: 0;">
<td width="100.00%" style="border: 0;" valign="top">

This page describes the Main toolbar and menu of [Substance 3D Designer](https://www.adobe.com/products/substance3d-designer.html), which appear at the top left of the main window.It consists of two parts: the drop-down main menus, and quick-access buttons. All quick access button functions can also be accessed through the <b>File</b> and <b>Edit</b> menu's.

</td>
<td width="41.67%" style="border: 0;" valign="top">

![Main toolbar](../../assets/mainmenu.png "Main toolbar")

</td>
</tr>
</table>

## Quick access buttons

![](../../assets/newsubstance.png)&nbsp;&nbsp;<b>New Substance graph...:</b> (Ctrl+N)Presents you the [New Substance graph](https://helpx.adobe.com/substance-3d/unlisted/documentation/sddoc/create-a-graph-102400068.html) window, then creates a new package with a [Substance graph](../../compositing-graphs/substance-compositing-graphs.md).

![](../../assets/open.png)&nbsp;&nbsp;<b>Open...:</b> (Ctrl+O) Open an existing [Substance package (.SBS, .SBSAR, .SBSASM)](../../getting-started/overview/overview.md).

![](../../assets/saveall.png)&nbsp;&nbsp;<b>Save All:</b> (Ctrl+⇧+S) Saves all packages listed in the [Explorer](../../interface/the-explorer-window/the-explorer-window.md).

![](../../assets/undo.png)&nbsp;&nbsp;<b>Undo:</b> (Ctrl+Z) Undo the last operation.

![](../../assets/redo.png)&nbsp;&nbsp;<b>Redo:</b> (Ctrl+Y) Redo the last undone operation.

## File

<b>New:</b> opens a sub menu to create a graph or package:

* <b>New Substance graph...:</b>(Ctrl+N) Presents you the [New Substance graph](https://helpx.adobe.com/substance-3d/unlisted/documentation/sddoc/create-a-graph-102400068.html) window which lets you set up a new [Substance graph](../../compositing-graphs/substance-compositing-graphs.md);
* <b>New Substance function graph:</b> Creates a new package with a [Substance function graph](../../function-graphs/function-graphs.md);
* <b>Empty:</b> Creates an empty package.

<b>Open...:</b> (Ctrl+O) Open an existing [Substance package (.SBS, .SBSAR, .SBSASM)](../../getting-started/overview/overview.md).

<b>Recent packages:</b> Displays a list of recently opened packages. Click on an entry to open it.

<b>Open last session packages (&#35;)</b>: Opens all packages that were open when the last session closed or was ended.

<b>Save all:</b> (Ctrl+⇧+S)  Saves all open packages, including packages loaded in the background.

<b>Close all:</b> Closes all open packages.

<b>Reload resources:</b> Forces Designer to reload [all resources, including bitmaps and SVG data](../../resources/importing-linking-and-new/importing-linking-and-new-resources.md).

<b>Exit:</b> (Ctrl+Q) - Close Substance 3D Designer.

## Edit

<b>Undo:</b> (Ctrl+Z) Undo last operation.

<b>Redo:</b> (Ctrl+Y) Redo last undone operation.

<b>Preferences...:</b> Opens the Preferences Window.

>[!NOTE]
>
> This dialog is accessed from the Substance 3D Designer menu in the taskbar on macOS.

## Tools

<b>Cancel rendering:</b> (Esc) Stops current operation for the Substance Engine. Can be used to abort an unwanted, heavy operation.

<b>Suspend engine:</b> (⇧+Esc) Suspends the rendering engine. This can speed up editing of complex [Substance graphs](../../compositing-graphs/substance-compositing-graphs.md).

<b>Switch engine...: </b>(F9) Offers a choice of rendering engines, including GPU engines ('DirectX' on Windows, 'OpenGL' on macOS) as well as the CPU engine ('NEON' on Apple Silicon, 'SSE' on all others).

<b>Substance Player:</b> Manage Designer's integration with Substance Player:

* <b>Locate Player...:</b> Let Designer know where Player is installed;
* <b>Download Player...:</b> Opens the Substance Player documentation's [landing page](https://helpx.adobe.com/substance-3d-player/home.html), where Player can be downloaded.

<b>Plugin Manager...</b>: Opens the Plugin Manager window, where you can install, load and unload [Python Plugins for Substance 3D Designer.](../../scripting/scripting.md)

## Windows

<b>New Explorer:</b> Opens a new Explorer dock. You can have multiple Explorer docks open.

<b>New 3D View:</b> Opens a new 3D View dock. You can have multiple 3D View docks open.

<b>New Library View:</b> Opens a new Library dock. You can have multiple Library docks open.

<b>Python Editor:</b> Opens the Python Editor used to[ evaluate and create scripts](../../scripting/scripting.md).

<b>Reset layout:</b> Resets the workspace to the default layout. All windows will be re-arranged, and some windows might be hidden again. Use in case of problems with program layout.

<b>Unmaximize window:</b> When any panel is *maximized*, this option unmaximizes it and restores the layout as it was *before* the window was maximized

<b>Explorer:</b> Show/Hide the [Explorer Window(s).](https://helpx.adobe.com/substance-3d/unlisted/documentation/sddoc/the-explorer-129368147.html)

<b>Graph:</b> Show/Hide the [graph Window](../../interface/the-graph-view/the-graph-view.md)(s).

<b>Parameters:</b> Show/Hide the [Properties Window](https://helpx.adobe.com/substance-3d/unlisted/documentation/sddoc/parameters-ui-129368153.html).

<b>Console:</b> Show/Hide the Console Window.

<b>3D View:</b> Show/Hide the [3D View(s)](../../interface/3d-view/3d-view.md).

<b>Dependency Manager:</b> Show/Hide the [Dependency Manager](../../interface/dependency-manager/dependency-manager.md).

<b>2D Views:</b> Show/Hide the [2D View(s).](https://helpx.adobe.com/substance-3d/unlisted/documentation/sddoc/2d-view-deprecated-129368155.html)

<b>Library:</b> Show/Hide the [Library Window.](../../interface/the-library/the-library.md)

<b>Main toolbar:</b> Show/Hide the Main Toolbar (quick-access buttons only).

>[!NOTE]
>
> To learn more about Designer's panel management, its customisation and workflow-enhancing features, go to the [Customizing your workspace ](../../interface/customizing-your-wor/customizing-your-workspace.md)page of this documentation.

## Help

<b>Tutorials:</b> Opens the [Substance 3D tutorials](https://substance3d.adobe.com/tutorials/) website (formerly Substance Academy).<b>  
</b>

<b>Release notes:</b> Opens a window with latest version's changelog.

<b>Technical requirements:</b> Shows you the technical requirements to run the application.

<b>Documentation:</b> Opens your default web browser on [this documentation](https://www.adobe.com/go/Substance-3D-doc-Designer).

<b>Scripting documentation:</b> Opens your web browser on the local Python API docs.

<b>Forums...:</b> Opens your web browser on our [Support Community](https://forum.substance3d.com/) forum to get in touch with the community and ask questions.

<b>Report a bug...:</b> Open bug reporting window.

<b>Export log...:</b> Exports current log files to a compressed (.zip) file, to provide to technical support.

<b>Give feedback...:</b> Opens your web browser on Adobe's [Support Community](https://www.adobe.com/go/Substance-3D-feedback-Designer) home page.

<b>Substance 3D assets:</b> Browse [premium 3D content](https://helpx.adobe.com/substance-3d/unlisted/assets.html) for subscribers (formerly Substance Source).

<b>Substance 3D community assets:</b> Lets you browse [free community assets](https://helpx.adobe.com/substance-3d/unlisted/community-assets.html) (formerly Substance Share).

<b>Manage my account\*:</b> Opens the webpage for your Adobe account.

<b>Sign in/Sign out...\*:</b> Lets you sign into/out of your Adobe account.

<b>Home screen...:</b> Displays the [Home screen](../../interface/home-screen/home-screen.md) dialog.

<b>What&#39;s new...:</b> Displays a screen, which highlights the features added to Designer's latest release

<b>Welcome screen...\*:</b> Displays the a screen which guides new users through Designer's purpose and its place in the [Substance 3D ecosystem](https://helpx.adobe.com/substance-3d.html)

<b>Partners:</b> Lets you access the disclaimers and notices for third-party integrations from our partners in Designer.

<b>About Substance 3D Designer...:</b> Displays information about the application and its components, such as version number.

\*: These options are only available in the version of Designer installed through [Adobe Creative Cloud Desktop](https://creativecloud.adobe.com/en/apps/download/creative-cloud), which requires a [Substance 3D subscription](https://www.adobe.com/creativecloud/plans.html?filter=3dar&amp;plan=individual).
