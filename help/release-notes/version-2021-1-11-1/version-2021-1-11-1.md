...
To disable a node, simply right-click on it and choose **Disable Selection** or press the keyboard shortcut **Shift+D**. Not all nodes can be disabled, it depends on several criteria. For more details see the dedicated section in the following [documentation page](../../help/interface/the-graph-view/the-graph-view.md).
...
To do so, simply use the **File &gt; Export** scene action in the **3D View**. For more details see the dedicated [documentation page](../../help/interface/3d-view/3d-view.md).
...
* **New settings to exclude content from the library**  
  The library can now exclude specific file extensions or specific name patterns (with the support of wildcards). This allows to ignore undesired content inside the [Library](../../help/interface/the-library/the-library.md) window.
