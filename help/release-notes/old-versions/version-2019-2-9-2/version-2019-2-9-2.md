...
* **Open the menu when click and dragging a link from a node**   
  When click and dragging the connection of a node and releasing the mouse it will now open the Node Creation menu. The node created will then be connected to the link. The nodes available in the list will be dependent of the incoming connection. ![](../../../assets/click-drag-insert-node.gif)
